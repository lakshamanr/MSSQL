...
";
    ...
    public static partial class SqlQueryConstant
    {
            ...
            @"
              ...
              SELECT 
                  CONVERT(NVARCHAR(MAX), 
                    'CREATE XML SCHEMA COLLECTION ' + QUOTENAME(@SchemaCollectionName) + ' AS ' + CHAR(13) + CHAR(10) +
                    'N''' + CAST((SELECT xml_schema_namespace(PARSENAME(@SchemaCollectionName, 2), PARSENAME(@SchemaCollectionName, 1))) AS NVARCHAR(MAX)) + ''''
                ) AS SQLScript,
    
                    ISNULL(ep.value, '') AS MS_Description  , 
                  STUFF((  
                    SELECT ', ' + QUOTENAME(PARSENAME(@SchemaCollectionName, 2)) + '.' + QUOTENAME(t.name) + '.' + QUOTENAME(c.name)  
                    FROM sys.tables AS t  
                    JOIN sys.columns AS c ON t.object_id = c.object_id  
                    JOIN sys.xml_schema_collections AS xsc2 ON c.xml_collection_id = xsc2.xml_collection_id  
                    WHERE xsc2.name = PARSENAME(@SchemaCollectionName, 1)  
                    FOR XML PATH(''), TYPE  
                ).value('.', 'NVARCHAR(MAX)'), 1, 2, '') AS DependentColumn
              FROM sys.xml_schema_collections xsc
              LEFT JOIN sys.extended_properties ep
                  ON xsc.xml_collection_id = ep.major_id   
              WHERE xsc.name =PARSENAME(@SchemaCollectionName, 1);
            ";
        ...
        public static string XMLSchemReference =
         ...
         WHERE  s.name=PARSENAME(@SchemaCollectionName,1)
        ";
    }
    public static partial class SqlQueryConstant
    {
